...
public class FeedController {
    ...
    private String getPushFeeds(Model model) {// 优势:实时性高，占用存储空间大
    ...
    private String getPullFeeds(Model model) {// 优势：占用的存储空间小，实时性低
        int localUserId = hostHolder.getUser() != null ? hostHolder.getUser().getId() : 0;
        List<Integer> followees = new ArrayList<>();
        if (localUserId != 0) {
            // 我关注的人
            // 僵尸号 登录的时候从新鲜事集合中 获取一次即可，而不用像推的方式每一条信息都会发很多份。
